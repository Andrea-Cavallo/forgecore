...
type WebhookVerifier interface {
	VerifyWebhook(rawBody []byte, sigHeader string) (*stripe.StripeEvent, error)
}
...
func (uc *HandleStripeWebhookUseCase) Execute(ctx context.Context, input HandleWebhookInput) error {
	...
	payment, err := uc.repo.GetByProviderID(ctx, domain.ProviderStripe, event.Data.Object.ID)
	if err != nil {
		return fmt.Errorf("pagamento non trovato per provider_id %s: %w", event.Data.Object.ID, err)
	}
