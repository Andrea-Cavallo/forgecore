...
type UserRepository interface {
	...
	GetByOAuthProvider(ctx context.Context, provider, providerID string, tenantID uuid.UUID) (*User, error)
...
type SessionRepository interface {
	...
	ListByUser(ctx context.Context, userID, tenantID uuid.UUID) ([]*Session, error)
	DeleteByID(ctx context.Context, id, tenantID uuid.UUID) error
...
type TokenStore interface {
	...
	StoreOneTimeToken(ctx context.Context, key, token string, ttlSeconds int64) error
	PopOneTimeToken(ctx context.Context, key string) (string, error)
}
