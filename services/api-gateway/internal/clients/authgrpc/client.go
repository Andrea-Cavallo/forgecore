// Package authgrpc provides a gRPC client for the auth-service ValidateToken RPC.
...
func init() {
	// Mirror the codec registered on the auth-service gRPC server.
...
// --- Request / Response types (must match auth-service server.go) ---
...
// Client wraps a gRPC connection to auth-service.
type Client struct {
	conn *grpc.ClientConn
}

// NewClient dials the auth-service gRPC endpoint at addr.
...
// ValidateToken calls auth-service.ValidateToken and returns the claims.
